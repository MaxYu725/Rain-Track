...
assert.match(sw, /const CACHE_VERSION = 'point-rain-pwa-v1\.6\.4-pwa34'/);
assert.ok(!sw.includes('const CORE_SHELL = ['), 'pwa34 must remain zero-prefetch');
assert.ok(sw.includes('event.waitUntil(self.skipWaiting())'), 'pwa34 install must remain zero-prefetch');
...
console.log('Boot watchdog isolation + pwa34 resilience PASS');
