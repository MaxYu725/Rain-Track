...
assert.ok(!sw.includes('const CORE_SHELL = ['), 'pwa61 must remain zero-prefetch');
assert.ok(sw.includes('event.waitUntil(self.skipWaiting())'), 'pwa61 install must remain zero-prefetch');
...
console.log('Boot isolation + optional Rain Home scale/Now + Next/chart polish + Radar current-frame analysis + pwa61 resilience PASS');
