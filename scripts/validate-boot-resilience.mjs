...
assert.match(sw, /const CACHE_VERSION = 'point-rain-pwa-v1\.6\.4-pwa46'/);
assert.ok(!sw.includes('const CORE_SHELL = ['), 'pwa46 must remain zero-prefetch');
assert.ok(sw.includes('event.waitUntil(self.skipWaiting())'), 'pwa46 install must remain zero-prefetch');
...
console.log('Boot watchdog isolation + pwa46 resilience PASS');
