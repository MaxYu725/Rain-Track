...
assert.match(sw, /const CACHE_VERSION = 'point-rain-pwa-v1\.6\.4-pwa44'/);
assert.ok(!sw.includes('const CORE_SHELL = ['), 'pwa44 must remain zero-prefetch');
assert.ok(sw.includes('event.waitUntil(self.skipWaiting())'), 'pwa44 install must remain zero-prefetch');
...
console.log('Boot watchdog isolation + pwa44 resilience PASS');
