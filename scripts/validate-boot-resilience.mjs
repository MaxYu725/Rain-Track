...
assert.ok(!sw.includes('const CORE_SHELL = ['), 'pwa53 must remain zero-prefetch');
assert.ok(sw.includes('event.waitUntil(self.skipWaiting())'), 'pwa53 install must remain zero-prefetch');
...
console.log('Boot isolation + optional Radar current-frame analysis + pwa53 resilience PASS');
