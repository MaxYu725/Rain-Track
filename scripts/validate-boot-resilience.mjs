...
assert.ok(!sw.includes('const CORE_SHELL = ['), 'pwa33 must remain zero-prefetch');
assert.ok(sw.includes('event.waitUntil(self.skipWaiting())'), 'pwa33 install must remain zero-prefetch');
...
console.log('Boot watchdog isolation + pwa33 resilience PASS');
