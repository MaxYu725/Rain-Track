...
assert.match(sw, /const CACHE_VERSION = 'point-rain-pwa-v1\.6\.4-pwa45'/);
assert.ok(!sw.includes('const CORE_SHELL = ['), 'pwa45 must remain zero-prefetch');
assert.ok(sw.includes('event.waitUntil(self.skipWaiting())'), 'pwa45 install must remain zero-prefetch');
...
console.log('Boot watchdog isolation + pwa45 resilience PASS');
