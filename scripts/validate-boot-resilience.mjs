...
assert.match(sw, /const CACHE_VERSION = 'point-rain-pwa-v1\.6\.4-pwa47'/);
assert.ok(!sw.includes('const CORE_SHELL = ['), 'pwa47 must remain zero-prefetch');
assert.ok(sw.includes('event.waitUntil(self.skipWaiting())'), 'pwa47 install must remain zero-prefetch');
...
console.log('Boot watchdog isolation + pwa47 resilience PASS');
