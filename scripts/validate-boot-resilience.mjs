...
assert.match(sw, /const CACHE_VERSION = 'point-rain-pwa-v1\.6\.4-pwa41'/);
assert.ok(!sw.includes('const CORE_SHELL = ['), 'pwa41 must remain zero-prefetch');
assert.ok(sw.includes('event.waitUntil(self.skipWaiting())'), 'pwa41 install must remain zero-prefetch');
...
console.log('Boot watchdog isolation + pwa41 resilience PASS');
