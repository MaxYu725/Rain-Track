...
assert.ok(!sw.includes('const CORE_SHELL = ['), 'pwa54 must remain zero-prefetch');
assert.ok(sw.includes('event.waitUntil(self.skipWaiting())'), 'pwa54 install must remain zero-prefetch');
...
console.log('Boot isolation + optional Rain Home chart intensity + Radar current-frame analysis + pwa54 resilience PASS');
