...
assert.match(sw, /const CACHE_VERSION = 'point-rain-pwa-v1\.6\.4-pwa37'/);
assert.ok(!sw.includes('const CORE_SHELL = ['), 'pwa37 must remain zero-prefetch');
assert.ok(sw.includes('event.waitUntil(self.skipWaiting())'), 'pwa37 install must remain zero-prefetch');
...
console.log('Boot watchdog isolation + pwa37 resilience PASS');
