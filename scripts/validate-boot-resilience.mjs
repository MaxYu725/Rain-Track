...
assert.match(sw, /const CACHE_VERSION = 'point-rain-pwa-v1\.6\.4-pwa38'/);
assert.ok(!sw.includes('const CORE_SHELL = ['), 'pwa38 must remain zero-prefetch');
assert.ok(sw.includes('event.waitUntil(self.skipWaiting())'), 'pwa38 install must remain zero-prefetch');
...
console.log('Boot watchdog isolation + pwa38 resilience PASS');
