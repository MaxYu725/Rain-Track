...
assert.ok(!sw.includes('const CORE_SHELL = ['), 'pwa48 must remain zero-prefetch');
assert.ok(sw.includes('event.waitUntil(self.skipWaiting())'), 'pwa48 install must remain zero-prefetch');
...
console.log('Boot watchdog isolation + pwa48 resilience PASS');
