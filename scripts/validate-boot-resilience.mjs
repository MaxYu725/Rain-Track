...
assert.match(sw, /const CACHE_VERSION = 'point-rain-pwa-v1\.6\.4-pwa42'/);
assert.ok(!sw.includes('const CORE_SHELL = ['), 'pwa42 must remain zero-prefetch');
assert.ok(sw.includes('event.waitUntil(self.skipWaiting())'), 'pwa42 install must remain zero-prefetch');
...
console.log('Boot watchdog isolation + pwa42 resilience PASS');
