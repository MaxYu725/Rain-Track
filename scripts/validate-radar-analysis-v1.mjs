...
const base = {
  rangeKm:64,
  heightKm:3,
  regional:{ echoCount:12, centroid:{ lat:22.2, lon:114.0 }, meanDistanceToLocationKm:30 },
  hongKong:{ sampleCount:100, echoCount:10, coverage:.1, meanStrength:1.8, maxStrength:2.4, centroid:{ lat:22.34, lon:114.30 } }
};
const history = summarizeRadarHistory([
  { ...base, time:'2026-08-20T08:30:00Z', nearby:{ sampleCount:20, echoCount:1, coverage:.03 } },
  { ...base, time:'2026-08-20T08:36:00Z', nearby:{ sampleCount:20, echoCount:3, coverage:.12 } },
  { ...base, time:'2026-08-20T08:42:00Z', nearby:{ sampleCount:20, echoCount:5, coverage:.24 } }
], { location:{ lat:22.32, lon:114.17 } });
assert.equal(history.focus, 'nearby');
assert.match(history.text, /所在地附近回波逐步增多/);

const described = describeRadarAnalysis(analysis, history, { locationName:'粉嶺' });
assert.match(described.hongKongText, /香港/);
assert.match(described.nearbyText, /粉嶺附近/);
assert.match(described.motionText, /回波逐步增多/);
...
for (const marker of [
  ...
  'summarizeRadarHistory(rows, { location:state.selected })'
]) assert.ok(runtime.includes(marker), `Radar analysis runtime marker missing: ${marker}`);

for (const forbidden of ['fetchRadarFrames', "api('/api/radar", 'setRadarIndex(', 'changeRadarRange(', 'changeRadarHeight(']) {
  assert.ok(!runtime.includes(forbidden), `Radar analysis must not become a second Radar controller: ${forbidden}`);
}
...
console.log('Radar client-side analysis + conservative motion + pwa52 regression gate PASS');
