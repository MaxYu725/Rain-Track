...
function ensureMapFirstTimelinePosition() {
  ...
  style.textContent = `
    ...
    #forecast-map-timeline.is-switching .forecast-play-button{opacity:.72}
    @media(max-width:700px){
      ...
      .forecast-product-title{font-size:.72rem}.forecast-info-button{width:23px;height:23px}.forecast-map-product-meta{margin-top:4px;font-size:.55rem}.forecast-map-info-note{right:0;width:min(360px,calc(100vw - 24px));font-size:.66rem;padding:7px 9px}
    }
  `;
...
function infoText(snapshot) {
  ...
  const selectedWindow = windowText(snapshot?.selectedFrame?.window);
  if (swirls) return `每 6 分鐘一個預報時間；雨量為截至該時間的 30 分鐘累積預測。本時段 ${selectedWindow}。`;
  return `目前使用 30 分鐘後備預報；雨量為所選時段的 30 分鐘累積預測。本時段 ${selectedWindow}。`;
}
...
function ensureMobileScrubber() {
  ...
  range?.addEventListener('change', () => {
    ...
    if (!Number.isInteger(targetIndex)) return syncMobileScrubber();
    if (targetIndex === snapshot.index) return syncMobileScrubber();
...
function bindMapInfoDismiss() {
  const mapContainer = document.getElementById('map-container');
  if (!mapContainer || mapContainer.dataset.forecastInfoDismissBound) return;
  mapContainer.dataset.forecastInfoDismissBound = 'true';
  mapContainer.addEventListener('pointerdown', event => {
    if (event.target.closest('#forecast-map-timeline,#rain-map-quickviews,#rain-map-area-summary,.rain-home-back-map')) return;
    hideForecastInfo();
  });
}
